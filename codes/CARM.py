...
from architecture import SPADEResnetBlock1 as SPADEResnetBlock
from encoder1 import ConvEncoder
...
class prior_upsampling(nn.Module):
    ...
    def forward(self, z):
        ...
        latent_2 = self.conv_latent_up2(latent_1) # 16
...
class msdi_net_new(nn.Module):
    ...
    def __init__(self, in_chn=3, wf=64, depth=5, relu_slope=0.2, hin_position_left=0, hin_position_right=4):
        ...
        # self.prior_upsampling.load_state_dict(torch.load("./MSDI_archs/pretrain_model/prior_upsampling.pth"), strict=False)

        self.net_prior = ConvEncoder()
        # self.net_prior.load_state_dict(torch.load("./MSDI_archs/pretrain_model/net_E.pth"), strict=False)
